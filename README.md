...
Всегда указывай `https://` явно. Команда без схемы сначала обращается к серверу по
незащищённому HTTP и только затем получает перенаправление на HTTPS.

Потоковый запуск `curl -fsSL https://ins.dinomiha.ru/dfish | bash` поддерживается:
установщик читает ответы непосредственно из терминала. Однако скачивание в файл
предпочтительнее — так можно проверить скрипт до запуска, а ошибка `curl` не будет
скрыта кодом завершения последней команды в pipeline.
